...
export class SQLiteService {
  ...
  async createKeyValueTable(): Promise<void> {
    await this.database.execute(
      `CREATE TABLE IF NOT EXISTS key_value (key TEXT PRIMARY KEY NOT NULL, value TEXT);`
    );
  }
  ...
  async openDatabase(
    ...
    const retCC = (await this.sqliteConnection.checkConnectionsConsistency())
      .result;
    const isConn = (await this.sqliteConnection.isConnection(DB_NAME, readonly))
      .result;
    if (retCC && isConn) {
  ...
  async storeData(key: string, value: any): Promise<void> {
    await this.database.execute(
      `INSERT OR REPLACE INTO key_value (key, value) VALUES ('${key}', '${JSON.stringify(value)}');`
    );
  }
  ...
  async clearData(): Promise<void> {
    await this.database.execute(`DELETE FROM key_value;`);
  }
}

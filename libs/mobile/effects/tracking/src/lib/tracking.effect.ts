...
export class TrackingEffects {
  startTracking$ = createEffect(() =>
    this._actions$.pipe(
      ...
      switchMap(() =>
        this._tracking.startTracking().pipe(
          map((enabled) =>
            enabled
              ...
              : TrackingActions.startTrackingFailure({
                  error: 'Could not start tracking services',
                })
          )
        )
      )
  ...
  stopTracking$ = createEffect(() =>
    this._actions$.pipe(
      ...
      switchMap(() =>
        this._storage.removeData(StorageKey.TRACKED_ACTIVITY).pipe(
          tap(() => console.log('REMOVED DATA')),
          switchMap(() =>
            this._tracking.stopTracking().pipe(
              map((enabled) =>
                !enabled
                  ? TrackingActions.stopTrackingSuccess()
                  : TrackingActions.stopTrackingFailure({
                      error: 'Could not stop tracking services',
                    })
              )
            )
          )
  ...
  storeActivity$ = createEffect(
    () =>
      this._actions$.pipe(
        ...
        switchMap(([_, activity]) =>
          this._storage.setData(StorageKey.TRACKED_ACTIVITY, activity)
        )
      ),
    { dispatch: false }
  );
